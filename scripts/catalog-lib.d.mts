...
export interface CatalogSource{schemaVersion:number;id:string;version:string;type:'font'|'effect'|'transition'|'text-animation';name:string;description:string;author:string;sourceUrl:string;source?:CatalogSourceDescriptor;license:string;licenseUrl?:string;tags:readonly string[];minAppVersion:string;maxAppVersion?:string;runtimeId?:string;preset?:Record<string,string|number|boolean>;preview:{kind:'image'|'video';file:string};font?:{family:string;style:'normal'|'italic';weight:number};changelog?:string;publishedAt?:string;files?:readonly unknown[];}
