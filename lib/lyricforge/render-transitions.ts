...
export interface TransitionRenderEnvironment{width:number;height:number;frameIndex:number;timeMs:number;}
...
const seeded=(t:TransitionRenderPlan,env:TransitionRenderEnvironment,salt=0)=>hashNoise(hashString(t.instanceId)^Math.imul(Math.round(env.timeMs)+1,0x45d9f3b)^salt);
...
const renderBlurDissolve:TransitionRenderHandler=(ctx,outgoing,incoming,t,env)=>{const progress=p(t),radius=n(t,'radius',24)*(t.quality==='simplified'?.5:1);ctx.save();ctx.globalAlpha=1-progress;ctx.filter=`blur(${radius*progress}px)`;draw(ctx,outgoing,env);ctx.restore();ctx.save();ctx.globalAlpha=progress;ctx.filter=`blur(${radius*(1-progress)}px)`;draw(ctx,incoming,env);ctx.restore();};
