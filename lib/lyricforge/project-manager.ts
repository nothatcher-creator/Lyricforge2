...
const dependency=z.object({id,type:z.enum(['font','effect','transition','text-animation','element']),version,sourceCatalogId:z.string().min(1).max(120).optional()});
...
export function validateProject(data:unknown):Project {
  ...
  for(const a of p.assets){if(a.type==='font'&&!a.fontFamily)throw new Error('An imported font is missing its family name.');if(a.catalogDependency&&a.catalogDependency.type!=='element')throw new Error('Only catalog elements may be attached to project media provenance.');}
