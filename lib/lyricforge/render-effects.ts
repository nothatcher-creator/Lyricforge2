...
function seeded(effect:ResolvedEffect,env:EffectRenderEnvironment,salt=0){return hashNoise(hashString(effect.instanceId)^Math.imul(env.frameIndex+1,0x45d9f3b)^salt);}
...
const renderBloom:EffectRenderHandler=(ctx,source,effect,env)=>{
  const radius=numberParam(effect,'radius',22),intensity=clamp01(numberParam(effect,'intensity',.45));
...
const renderSharpen:EffectRenderHandler=(ctx,source,effect,env)=>withFilter(ctx,source,env,`contrast(${1+numberParam(effect,'amount',.4)*.3}) saturate(${1+numberParam(effect,'amount',.4)*.08})`);
...
const renderGrain:EffectRenderHandler=(ctx,source,effect,env)=>{
  ...
  const size=Math.max(1,numberParam(effect,'size',1));const cells=Math.min(900,Math.ceil(env.width*env.height/Math.max(1,size*size*250)));
...
const renderPixelate:EffectRenderHandler=(ctx,source,effect,env)=>{
  const size=Math.max(1,Math.round(numberParam(effect,'size',8)));
...
const renderVhs:EffectRenderHandler=(ctx,source,effect,env)=>{
  ...
  ctx.save();ctx.globalAlpha=scanlines*.25;ctx.fillStyle='#000000';for(let y=0;y<env.height;y+=4)ctx.fillRect(0,y,env.width,1);ctx.globalAlpha=noise*.2;ctx.fillStyle=seeded(effect,env,99)>.5?'#ffffff':'#000000';for(let i=0;i<60;i++){ctx.fillRect(seeded(effect,env,i*31)*env.width,seeded(effect,env,i*47+7)*env.height,1,1);}ctx.restore();
};
const renderNoiseDisplacement:EffectRenderHandler=(ctx,source,effect,env)=>{
  const amount=numberParam(effect,'amount',8),band=Math.max(2,Math.round(numberParam(effect,'scale',24)));for(let y=0,i=0;y<env.height;y+=band,i++){const h=Math.min(band,env.height-y),dx=(seeded(effect,env,i)-.5)*amount*2;ctx.drawImage(source,0,y,env.width,h,dx,y,env.width,h);}
};
const renderShake:EffectRenderHandler=(ctx,source,effect,env)=>{
  const amount=numberParam(effect,'amount',8),speed=numberParam(effect,'speed',8),phase=env.frameIndex*Math.max(.01,speed)*.13;ctx.save();ctx.translate(Math.sin(phase)*amount,Math.cos(phase*1.37)*amount);draw(ctx,source,env);ctx.restore();
};
const renderZoomPulse:EffectRenderHandler=(ctx,source,effect,env)=>{
  const amount=numberParam(effect,'amount',.08),period=Math.max(100,numberParam(effect,'periodMs',800)),t=env.frameIndex*1000/60,scale=1+Math.sin(t/period*Math.PI*2)*amount;ctx.save();centerTransform(ctx,env,scale);draw(ctx,source,env);ctx.restore();
};
...
const renderGlitch:EffectRenderHandler=(ctx,source,effect,env)=>{
  const intensity=clamp01(numberParam(effect,'intensity',.35)),rate=clamp01(numberParam(effect,'rate',.25));if(seeded(effect,env,5)>rate){draw(ctx,source,env);return;}draw(ctx,source,env);const slices=Math.max(2,Math.round(3+intensity*10));for(let i=0;i<slices;i++){const y=Math.floor(seeded(effect,env,i*19)*env.height),h=Math.max(1,Math.floor(seeded(effect,env,i*23+3)*18+2)),dx=(seeded(effect,env,i*29+7)-.5)*intensity*80;ctx.drawImage(source,0,y,env.width,h,dx,y,env.width,h);}
};
