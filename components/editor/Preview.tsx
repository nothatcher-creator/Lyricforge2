...
type PreviewProps={onFormat:()=>void;onLeft:()=>void;onRight:()=>void;onTextSelected?:(id:string)=>void};
export default function Preview({onFormat,onLeft,onRight,onTextSelected}:PreviewProps){
  ...
  useEffect(()=>{
    const render=new Renderer();renderer.current=render;let raf=0;let lastT=-1,lastP=project,lastSel=selected,lastGuide=false;const el=canvas.current!;
