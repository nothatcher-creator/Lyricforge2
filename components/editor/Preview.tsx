...
type PreviewProps={onFormat:()=>void;onLeft:()=>void;onRight:()=>void;onOpenProperties?:(id:string)=>void;quality:PreviewQuality;onQualityChange:(quality:PreviewQuality)=>void};
export default function Preview({onFormat,onLeft,onRight,onOpenProperties,quality,onQualityChange}:PreviewProps){
  ...
  const pointerDown=(e:React.PointerEvent<HTMLCanvasElement>)=>{
    ...
    const s=effectiveStyle(project,clip);const startX=e.clientX,startY=e.clientY;const startedAt=performance.now();const pointerType=e.pointerType;
